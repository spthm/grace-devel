...
public :: src_ray_pluecker_float

public :: cpp_src_ray_type
public :: slope_src_ray_type

real(r4b), parameter :: zero = 0.0d0
real(r4b), parameter :: one = 1.0d0


!> basic ray type
!-----------------------------------------------------------------------
type, BIND(C) :: src_ray_type
   real(C_FLOAT) :: start(3)  !< starting position
   real(C_FLOAT) :: dir(3)    !< unit vector direction
   real(C_FLOAT) :: length    !< length (determines when to stop tracing)
   integer(C_INT) :: class  !< based on direction signs (MMM, PMM, ...)

   real(C_FLOAT) :: freq      !< freq in HI ionizing units
   real(C_FLOAT) :: enrg      !< enrg of a single photon in ergs
   real(C_FLOAT) :: pcnt      !< photon count (changes as the ray is depleted)
   real(C_FLOAT) :: pini      !< initial photons
   real(C_FLOAT) :: dt_s      !< time step associated with ray [s]
...
type, BIND(C) :: slope_src_ray_type
   real(C_FLOAT) :: x, y, z  !< starting position
   real(C_FLOAT) :: i, j, k    !< unit vector direction
   real(C_FLOAT)  :: ibyj, jbyi, kbyj, jbyk, ibyk, kbyi !< Slope
   real(C_FLOAT)  :: c_xy, c_xz, c_yx, c_yz, c_zx, c_zy
   real(C_FLOAT) :: length    !< length (determines when to stop tracing)
   integer(C_INT) :: classification  !< based on direction signs (MMM, PMM, ...)
...
!-----------------------------------------------------------------------
  subroutine src_ray_make(ray)
    ...
    do while ( r .GT. 1.0d0 .and. r .NE. 0.0d0 )
       xx=sngl(2.0d0 * genrand_real1()-1.0d0)
       yy=sngl(2.0d0 * genrand_real1()-1.0d0)
       zz=sngl(2.0d0 * genrand_real1()-1.0d0)
       r=xx*xx+yy*yy+zz*zz
    enddo
    r = sqrt(r)
...
!-----------------------------------------------------
function src_ray_pluecker_float(src_ray, s2b, s2t) result( hit )

  type(src_ray_type), intent(in) :: src_ray

  real(r4b) :: s2b(3)         !< vector from ray start to lower cell corner
  real(r4b) :: s2t(3)         !< vector from ray start to upper cell corner
  logical :: hit              !< true or false result

  real(r4b) :: dir(3)
  real(r4b) :: dist

  real(r4b) :: e2b(3)       !< vector from ray end to lower cell corner
  real(r4b) :: e2t(3)       !< vector from ray end to upper cell corner
...
end function src_ray_pluecker_float
