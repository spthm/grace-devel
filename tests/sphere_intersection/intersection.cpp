...
double impact_parameter2(const grace::Ray& ray,
                         const grace::Sphere<double>& sphere)
{
...
float impact_parameter2(const grace::Ray& ray,
                        const grace::Sphere<float>& sphere)
{
    mpq_class dx = mpq_class(ray.dx);
    mpq_class dy = mpq_class(ray.dy);
    mpq_class dz = mpq_class(ray.dz);
    ...
    return static_cast<float>(b2.get_d());
}
