...
int main(int argc, char* argv[])
{
    ...
    grace::AABB<double> aabb;
    aabb.max = grace::Vector<3, double>(1., 1., 1.);
    aabb.min = grace::Vector<3, double>(-1., -1., -1.);
    thrust::host_vector<grace::Vector<3, double> > h_points(N);
    thrust::transform(thrust::counting_iterator<size_t>(0),
                      thrust::counting_iterator<size_t>(N),
                      h_points.begin(),
                      random_vector3_functor<double>(aabb.min, aabb.max));
    ...
    grace::morton_keys(d_points, aabb, d_keys, grace::CentroidSphere<double>());
