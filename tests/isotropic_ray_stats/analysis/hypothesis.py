...
def _indicator(x, y):
    return np.trunc(0.5 * (np.sign(x - y) + 1)).astype(int)
...
def _welch_satterthwaite_dof(s2x, s2y, nx, ny):
    ...
    if np.isnan(dof):
        ...
        print("s2x: %.6f\ns2y: %.6f\nnx: %d\nny: %d" % (s2x, s2y, nx, ny))
        dof = (nx + ny) / 2.
        warn("Welch Satterthwaite DoF is Nan", RuntimeWarning)
    return dof
...
def _mean_diff_confidence_interval(X, Y, cl):
    ...
    if s2x == 0 and s2y == 0:
        # Typically occurs when the input data are all zero
        warn("Variances of X and Y are 0. Cannot compute confidence interval",
             RuntimeWarning)
        return (0., 0.)
...
class _MW_Worker(Process):
    def __init__(self, X, Y, inqueue, outque):
        super(_MW_Worker, self).__init__()
        self.X = X
        self.Y = Y
        self.m = len(X)
        self.n = len(Y)
    ...
    def run(self):
        ...
        for (task_name, idx_range) in iter(self.inq.get, None):
            task = getattr(self, task_name)
            result = task(*idx_range)
            self.oq.put((task_name, result))

    def wxy(self, ilow, ihigh):
        partials = []
        for i in range(ilow, ihigh):
            x = self.X[i]
            psum = math.fsum(self._indicator(x, self.Y))
            partials.append(psum)
        return math.fsum(partials) / float(self.m * self.n)

    def wxxy(self, ilow, ihigh):
        partials = []
        # For even [ilow, ihigh] ranges, this does NOT result in a balanced
        # workload!
        for i1 in range(ilow, ihigh):
            for i2 in range(i1 + 1, self.m):
                x = min(self.X[i1], self.X[i2])
                psum = math.fsum(self._indicator(x, self.Y))
                partials.append(psum)
        return math.fsum(partials) * 2. / (self.m * (self.m - 1) * self.n)

    def wxyy(self, ilow, ihigh):
        partials = []
        for i in range(ilow, ihigh):
            x = self.X[i]
            for j1 in range(self.n):
                y = np.maximum(self.Y[j1], self.Y[j1 + 1:])
                psum = math.fsum(self._indicator(x, y))
                partials.append(psum)
        return math.fsum(partials) * 2. / (self.n * (self.n - 1) * self.m)

    def _indicator(self, x, y):
        return np.trunc(0.5 * (np.sign(x - y) + 1)).astype(int)

def _mann_whitney(X, Y, nprocs=1):
    """
    ...
    m = len(X)
    n = len(Y)
    nperw = (m + nprocs - 1) // nprocs

    in_queue = Queue()
    out_queue = Queue()

    for i in range(nprocs):
        # Not a particularly even work distribution...
        ilow = i * nperw
        ihigh = min((i + 1)*nperw, m)
        in_queue.put(('wxy', (ilow, ihigh)))
        in_queue.put(('wxxy', (ilow, ihigh)))
        in_queue.put(('wxyy', (ilow, ihigh)))

    workers = []
    for i in range(nprocs):
        w = _MW_Worker(X, Y, in_queue, out_queue)
        workers.append(w)
        w.start()

    # Exit sentinels for workers.
    for i in range(nprocs):
        in_queue.put(None)

    results = {'wxy': 0, 'wxxy': 0, 'wxyy': 0}
    done = 0
    while done < nprocs * len(results):
        try:
            (task, val) = out_queue.get(timeout=10)
            results[task] += val
            done += 1
        except QueueEmpty:
            pass
        except KeyboardInterrupt:
            print("Detected CTRL+C, terminating worker processes...")
            for w in workers:
                w.terminate()
            for w in workers:
                w.join()
    ...
    wxxy = results['wxxy']
    wxyy = results['wxyy']
    sxy2 = wxy - (m + n - 1)*wxy*wxy + (m - 1)*wxxy + (n - 1)*wxyy
    sxy2 *= 1. / (m * n)
    sxy = np.sqrt(sxy2)
...
def equivalence_test(X, Y, e1, e2, cl=0.95):
    ...
    X is the test distribution and Y is the reference distribution.
