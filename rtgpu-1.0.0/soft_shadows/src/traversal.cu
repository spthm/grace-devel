...
{
    int idfrustum = (blockIdx.y*gridDim.x+blockIdx.x)*blockDim.x + threadIdx.x;
    ...
    typename F::frustum_type frustum = frustums.to_frustum(idfrustum);
                        ...
                        int depth)
{
    ...
    //std::cout << "Grid: " << dimGrid << std::endl;

    typename F::const_gpu_type fgpu = frustums;

#if MAX_DEPTH
    if(F::has_near && depth+1 == MAX_DEPTH)
    {
        cuda_timer timer;
                       ...
                       const dvector<unsigned> &d_ray_packet_sizes)
{
    ...
    dvector<unsigned> d_in_frustum_ids, d_in_node_ids,
                             d_out_frustum_ids,
                             d_num_intersected,
                             d_dest_positions,
...
//    std::cout << "-------------------------------------------------\n";
//    std::cout << "FRUSTUMS: " << d_frustums.size() << std::endl;

    // first and intermediate levels
    for(int i=0; i<bvh_height-1; ++i)
    {
//        std::cout << "IN FRUSTUMS: " << d_in_frustum_ids.size() << std::endl;
        ...
        if(d_out_frustum_ids.empty())
        {
            d_active_ray_packets.clear();
            d_active_ray_packet_sizes.clear();
            d_active_frustum_leaves.clear();
            d_active_frustum_leaf_sizes.clear();
            d_active_idleaves.clear();
            return;
        }
...
//    std::cout << "IN FRUSTUMS: " << d_in_frustum_ids.size() << std::endl;
    ...
    // last level
    dvector<unsigned> d_reserved_children_space;
...
//    std::cout << "OUT FRUSTUMS: " << d_out_frustum_ids.size() << std::endl;
    ...
    dvector<unsigned> d_active_idfrustums;
