                                ...
                                unsigned num_packets)
{
...
#if 0
        // topleft
        V = unit(make_float3(far_plane, far_u_max, far_v_max) -
                 make_float3(near_plane, near_u_max, near_v_max));
        V2 = unit(cross(make_float3(frustums.left[idfrustum]),
                       make_float3(frustums.top[idfrustum])));
        cuPrintf("%d 1 TL:\t%f\t%f\t%f - %f,%f,%f\n",idfrustum,V.x,V.y,V.z,
                                                               V.x-V2.x,V.y-V2.y,V.z-V2.z);


        // topright
        V = unit(make_float3(far_plane, far_u_min, far_v_max) -
                 make_float3(near_plane, near_u_min, near_v_max));
        V2 = unit(cross(make_float3(frustums.top[idfrustum]),
                       make_float3(frustums.right[idfrustum])));
        cuPrintf("%d 1 TR:\t%f\t%f\t%f - %f,%f,%f\n",idfrustum,V.x,V.y,V.z,
                                                               V.x-V2.x,V.y-V2.y,V.z-V2.z);


        // bottomright
        V = unit(make_float3(far_plane, far_u_min, far_v_min) -
                 make_float3(near_plane, near_u_min, near_v_min));
        V2 = unit(cross(make_float3(frustums.right[idfrustum]),
                       make_float3(frustums.bottom[idfrustum])));
        cuPrintf("%d 1 BR:\t%f\t%f\t%f - %f,%f,%f\n",idfrustum,V.x,V.y,V.z,
                                                               V.x-V2.x,V.y-V2.y,V.z-V2.z);
        

        // bottomleft
        V = unit(make_float3(far_plane, far_u_max, far_v_min) -
                 make_float3(near_plane, near_u_max, near_v_min));
        V2 = unit(cross(make_float3(frustums.bottom[idfrustum]),
                       make_float3(frustums.left[idfrustum])));
        cuPrintf("%d 1 BL:\t%f\t%f\t%f - %f,%f,%f\n",idfrustum,V.x,V.y,V.z,
                                                               V.x-V2.x,V.y-V2.y,V.z-V2.z);
#endif
...
#if 0
        frustums.ori[idfrustum] = make_float3(near_plane, 
                                              (near_u_max+near_u_min)/2,
    ...
    case -1: // -X is major axis, right is +Y, top is +Z
        ...
        cuPrintf("%d TOP: %f,%f,%f - TR: %f,%f,%f\n",idfrustum, frustums.top[idfrustum].x,
                                   frustums.top[idfrustum].y,
                                   frustums.top[idfrustum].z,
                                   far_plane-near_plane,
                                   far_u_max-near_u_max,
                                   far_v_max-near_v_max);


        cuPrintf("%d RIGHT: %f,%f,%f - BR: %f,%f,%f,\n",idfrustum, frustums.right[idfrustum].x,
                                   frustums.right[idfrustum].y,
                                   frustums.right[idfrustum].z,
                                   far_plane-near_plane,
                                   far_u_max-near_u_max,
                                   far_v_min-near_v_min);

        cuPrintf("%d BOTTOM: %f,%f,%f - BL: %f,%f,%f\n",idfrustum, frustums.bottom[idfrustum].x,
                                   frustums.bottom[idfrustum].y,
                                   frustums.bottom[idfrustum].z,
                                   far_plane-near_plane,
                                   far_u_min-near_u_min,
                                   far_v_min-near_v_min);

        cuPrintf("%d LEFT: %f,%f,%f - TL: %f,%f,%f\n",idfrustum, frustums.left[idfrustum].x,
                                   frustums.left[idfrustum].y,
                                   frustums.left[idfrustum].z,
                                   far_plane-near_plane,
                                   far_u_min-near_u_min,
                                   far_v_max-near_v_max);
#endif
    ...
    case 2: // Y is major axis, right is +X, top is +Z
        ...
        // topleft
        V = unit(make_float3(far_u_max, far_plane, far_v_max) -
                 make_float3(near_u_max, near_plane, near_v_max));
        V2 = unit(cross(make_float3(frustums.left[idfrustum]),
                       make_float3(frustums.top[idfrustum])));
        cuPrintf("%d 2 TL:\t%f\t%f\t%f - %f,%f,%f\n",idfrustum,V.x,V.y,V.z,
                                                               V2.x,V2.y,V2.z);


        // topright
        V = unit(make_float3(far_u_min, far_plane, far_v_max) -
                 make_float3(near_u_min, near_plane, near_v_max));
        V2 = unit(cross(make_float3(frustums.top[idfrustum]),
                       make_float3(frustums.right[idfrustum])));
        cuPrintf("%d 2 TR:\t%f\t%f\t%f - %f,%f,%f\n",idfrustum,V.x,V.y,V.z,
                                                               V2.x,V2.y,V2.z);


        // bottomright
        V = unit(make_float3(far_u_min, far_plane, far_v_min) -
                 make_float3(near_u_min, near_plane, near_v_min));
        V2 = unit(cross(make_float3(frustums.right[idfrustum]),
                       make_float3(frustums.bottom[idfrustum])));
        cuPrintf("%d 2 BR:\t%f\t%f\t%f - %f,%f,%f\n",idfrustum,V.x,V.y,V.z,
                                                                  V2.x,V2.y,V2.z);
        

        // bottomleft
        V = unit(make_float3(far_u_max, far_plane, far_v_min) -
                 make_float3(near_u_max, near_plane, near_v_min));
        V2 = unit(cross(make_float3(frustums.bottom[idfrustum]),
                       make_float3(frustums.left[idfrustum])));
        cuPrintf("%d 2 BL:\t%f\t%f\t%f - %f,%f,%f\n",idfrustum,V.x,V.y,V.z,
                                                                  V2.x,V2.y,V2.z);
#endif
...
#if 0
        frustums.ori[idfrustum] = make_float3((near_u_max+near_u_min)/2,
                                              near_plane,
                                              (near_v_max+near_v_min)/2);
#endif
        break;
    ...
    case 3: // Z is major axis, right is -X, top is +Y
        ...
        // topleft
        V = unit(make_float3(far_u_max, far_v_max, far_plane) -
                 make_float3(near_u_max, near_v_max, near_plane));
        V = make_float3(frustums.top[idfrustum]);
        cuPrintf("%d 3 TL:\t%f\t%f\t%f\n",idfrustum,V.x,V.y,V.z);


        // topright
        V = unit(make_float3(far_u_min, far_v_max, far_plane) -
                 make_float3(near_u_min, near_v_max, near_plane));
        V = make_float3(frustums.right[idfrustum]);
        cuPrintf("%d 3 TR:\t%f\t%f\t%f\n",idfrustum,V.x,V.y,V.z);


        // bottomright
        V = unit(make_float3(far_u_min, far_v_min, far_plane) -
                 make_float3(near_u_min, near_v_min, near_plane));
        V = make_float3(frustums.bottom[idfrustum]);
        cuPrintf("%d 3 BR:\t%f\t%f\t%f\n",idfrustum,V.x,V.y,V.z);
        

        // bottomleft
        V = unit(make_float3(far_u_max, far_v_min, far_plane) -
                 make_float3(near_u_max, near_v_min, near_plane));
        V = make_float3(frustums.left[idfrustum]);
        cuPrintf("%d 3 BL:\t%f\t%f\t%f\n",idfrustum,V.x,V.y,V.z);
#endif
                          ...
                          dvector<float3> &normal)
{
    ...
    static dvector<unsigned> dest_idx;
    scan_add(dest_idx, mask, EXCLUSIVE);

    int len = dest_idx.back() + mask.back();

    static dvector<int> out_idx;
    static dvector<float3> out_pos, out_normal;
    out_idx.resize(len);
    out_pos.resize(len);
...
__global__ void shadow_phong_shade(float4 *output, /*{{{*/
                      const int *idrays,
...
__host__ void shadow_phong_shade(float4 *d_output,
                    ...
                    const dvector<unsigned> &d_on_shadow)
{
    ...
    assert(d_positions.size() == d_on_shadow.size());
    ...
    shadow_phong_shade<<<dimGrid, dimBlock>>>(d_output,
                         ...
                         bool reload_model)
{
    ...
    if(true || d_rays_dir.empty() || recalc_rays || 
...
//    cuda_timer timer;
    ...
    static dvector<float3> d_isect_positions;
    static dvector<float3> d_isect_normals;
    static dvector<int> d_isect_rays_idx;
    ...
    static dvector<float3> d_shadow_rays_dir;
    float cell_size = create_shadow_rays(d_shadow_rays_dir, d_rays_hash, 
                       bvh[0].aabb, d_isect_positions);

    static dvector<unsigned> d_shadow_ray_packets, d_shadow_ray_packet_sizes,
                             d_shadow_rays_idx;

    int bits = ceil(log2(1/cell_size))*3;

//    std::cout << "---------------------------------------------------" << std::endl;

    create_ray_packets(d_shadow_ray_packets, d_shadow_ray_packet_sizes,
                       d_shadow_rays_idx,
                       d_rays_hash, d_shadow_rays_dir, bits);
#if 0

    dim3 dimBlock(256),
         dimGrid((d_shadow_ray_packets.size()+255)/256);
    shadow_packet_trace<<<dimGrid, dimBlock>>>(d_isect_rays_idx,
                                               d_shadow_rays_idx,
                                               d_shadow_ray_packets,
                                               d_shadow_ray_packet_sizes,
                                               d_shadow_ray_packets.size(),
                                               d_output);
#endif

#if 0
    std::vector<unsigned> h_ray_packets = to_cpu(d_shadow_ray_packets), 
                          h_packet_sizes = to_cpu(d_shadow_ray_packet_sizes);
    for(int i=0; i<h_ray_packets.size(); ++i)
    {
        std::cout << i << ": " << h_ray_packets[i] << " - " << h_packet_sizes[i] << std::endl;
    }
    ...
    static FrustumsOri d_shadow_frustums;
    create_frustums(d_shadow_frustums, bvh[0].aabb,
                    d_shadow_rays_idx, d_isect_positions, d_shadow_rays_dir, 
                    d_shadow_ray_packets, d_shadow_ray_packet_sizes);

#if 0
    std::vector<unsigned> aux = to_cpu(d_shadow_ray_packet_sizes);
    for(int i=0; i<aux.size(); ++i)
    {
        std::cout << aux[i] << ", ";
    }
...
//    exit(0);
    ...
    {
        static dvector<unsigned> d_on_shadow;
        ...
        shadow_local_intersections(d_on_shadow,
                            ...
                            d_bvh,
                            d_tri_xform,
                            d_isect_positions,d_shadow_rays_dir,d_shadow_rays_idx);

#if 0

        dim3 dimBlock(256),
             dimGrid((d_active_ray_packets.size()+255)/256);
        shadow_packet_trace<<<dimGrid, dimBlock>>>(d_on_shadow,
                                                   d_isect_rays_idx,
                                                   d_shadow_rays_idx,
                                                   d_active_ray_packets,
                                                   d_active_ray_packet_sizes,
                                                   d_active_ray_packets.size(),
                                                   d_output);
#endif
#if 1
        shadow_phong_shade(d_output,
