...
#if 0
    m_cuda_output_buffer = m_ctx->createBufferFromGLBO(RT_BUFFER_OUTPUT, pbo);
    m_cuda_output_buffer->setFormat(RT_FORMAT_FLOAT4);
    m_ctx["output_buffer"]->set(m_cuda_output_buffer);
#endif
...
void canvas::update_scene()/*{{{*/
{
    ...
    auto model_rot = to_quaternion(r3::y_axis, M_PI/4);

    for(unsigned i=0; i<model.positions.size(); ++i)
    {
        model.positions[i] = (model.positions[i] - centroid(bbox))/
                             bbox.size[maxdim]*scale;
//        model.positions[i] = rot(model.positions[i],model_rot);
//        model.normals[i] = rot(model.normals[i],model_rot);
    }
...
//    std::cout << "LIGHT: " << light_pos << std::endl;
    ...
    cuda_trace(cuda_u, cuda_v, cuda_w, cuda_invu, cuda_invv, cuda_invw,
               ...
               m_canvas_size.w, m_canvas_size.h, 
               m_scene_dirty);
