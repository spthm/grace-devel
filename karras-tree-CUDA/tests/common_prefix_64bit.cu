...
int main(int argc, char* argv[]) {
    ...
    thrust::device_vector<grace::uinteger64> d_keys = h_keys;
    ...
    common_prefix_kernel<<<1,d_prefix_lengths.size()>>>(
        ...
        (grace::uinteger64*)thrust::raw_pointer_cast(d_keys.data()),
        d_keys.size(),
    ...
    for (int i=0; i<4; i++) {
        ...
        std::cout << "keys[i]: " << (std::bitset<64>) h_keys[h_is[i]]
                  << std::endl;
        std::cout << "keys[j]: " << (std::bitset<64>) h_keys[h_js[i]]
                  << std::endl;
