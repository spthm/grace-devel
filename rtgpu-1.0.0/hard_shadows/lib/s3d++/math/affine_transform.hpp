...
AffineTransform<T,D>::AffineTransform(const std::initializer_list<Vector<T,D+1>> &rows)/*{{{*/
	: LinearTransform<T,D>(typename base::init_base_xform_tag(),
						   identity_augment<D+1,D+1>(Matrix<T,D,D+1>(rows)))
{
}/*}}}*/
...
// $Id: affine_transform.hpp 3205 2010-11-15 02:23:59Z rodolfo $
