    ...
    -> std::shared_ptr<bvh_node> 
{
    ...
    struct partition_callback
    {
        static bool call(const prim_info &prim, size_t bucket_count,
                         int split_dim, int min_cost_split,
                         const r3::box &centroid_bounds)
        {
    ...
    auto pmid = std::partition(build_data.begin()+start, build_data.begin()+end,
            std::bind(&partition_callback::call, p::_1, bucket_count, split_dim,
                      min_cost_split, std::ref(centroid_bounds)));
    ...
    -> std::vector<linear_8bvh_node>
{
    ...
    for(int i=0; i<linear_bvh.size(); ++i)
    {
        ...
        else
        {
            ...
            {
                static bool call(const std::shared_ptr<bvh_node> &a, 
