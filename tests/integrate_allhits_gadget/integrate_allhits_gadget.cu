...
int main(int argc, char* argv[])
{
    ...
    thrust::device_vector<int> d_offsets(N_rays);
    thrust::device_vector<int> d_indices;
    thrust::device_vector<float> d_integrals;
    ...
    plane_parallel_rays_z(N_per_side, mins, maxs, rng_states, d_rays,
                          &area_per_ray);
    grace::trace_sph(d_rays, d_spheres, d_tree, d_offsets, d_indices,
                     d_integrals, d_distances);
